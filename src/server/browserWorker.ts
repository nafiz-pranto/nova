...
import { DEFAULT_MODEL_V1, QualificationEngine } from '../utils/qualificationEngine';
import { 
  isValidLocationCode, 
  getLocationByCode, 
...
export async function runBrowserWorker(
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      emitEvent({
        ...
        totalLimit: totalToExtract,
        currentEntityName: item.pageName,
        logMessage: `[00:00:${String(5 + i * 2).padStart(2, '0')}] Extracted DOM token for Library ID ${item.adLibraryId}. Page: "${item.pageName}". Destination: ${item.destinationUrl || 'None'}`
      });
      ...
      emitEvent({
        ...
        totalLimit: totalToExtract,
        currentEntityName: item.pageName,
        logMessage: `[00:00:${String(6 + i * 2).padStart(2, '0')}]  `
      });
      ...
      const advRecord: AdvertiserViewModel = {
        ...
        destinationDomain: item.destinationUrl,
