...
import { PHASE_03_SECTIONS } from '../data/phase03Sections';
import { PHASE_03_CONTENT } from '../data/phase03Content';
import { Search, ChevronLeft, ChevronRight, BookOpen, Layers, ShieldCheck } from 'lucide-react';

export const Phase03Reader: React.FC = () => {
  const [activeSectionId, setActiveSectionId] = useState<string>('p3-traceability');
  const [searchTerm, setSearchTerm] = useState<string>('');
  const [selectedCategory, setSelectedCategory] = useState<string>('all');

  const filteredSections = PHASE_03_SECTIONS.filter((s) => {
    const matchesSearch =
  ...
  const currentIndex = PHASE_03_SECTIONS.findIndex((s) => s.id === activeSectionId);
  const currentSection = PHASE_03_SECTIONS[currentIndex] || PHASE_03_SECTIONS[0];
  const sectionContent = PHASE_03_CONTENT[activeSectionId] || {
    title: currentSection.title,
    content: `Detailed specifications for ${currentSection.title} are fully documented in the authoritative Phase 03 blueprint.`
  };

  const prevSection = currentIndex > 0 ? PHASE_03_SECTIONS[currentIndex - 1] : null;
  const nextSection = currentIndex < PHASE_03_SECTIONS.length - 1 ? PHASE_03_SECTIONS[currentIndex + 1] : null;
      ...
      <aside className="w-80 border-r border-neutral-200 bg-neutral-50/50 flex flex-col h-full">
        <div className="p-4 border-b border-neutral-200 bg-white">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search 23 Phase 03 sections..."
              value={searchTerm}
          ...
          <div className="flex items-center justify-between mt-2.5 text-[11px] text-neutral-500 font-mono">
            <span>{filteredSections.length} of {PHASE_03_SECTIONS.length} SECTIONS</span>
            <span className="text-emerald-600 font-medium">PHASE 03 SPEC</span>
          </div>

          {/* Category Pill Filters */}
          <div className="flex flex-wrap gap-1 mt-2.5 pt-2 border-t border-neutral-100">
            {categories.map((cat) => (
              <button
                key={cat.id}
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="truncate leading-tight font-medium">{section.title.replace(/^#\s*\d+\.\s*/, '')}</p>
      ...
      <div className="flex-1 flex flex-col h-full bg-white overflow-y-auto p-8 max-w-4xl mx-auto">
        <div className="border-b border-neutral-200 pb-6 mb-6">
          <div className="flex items-center gap-2 text-xs font-mono text-neutral-500 mb-2">
            <span>PHASE 03 • SECTION {currentSection.number.toString().padStart(2, '0')} OF 23</span>
            <span>•</span>
            <span className="uppercase tracking-wider font-semibold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200">
        ...
        <div className="prose prose-neutral max-w-none text-neutral-800 text-sm leading-relaxed space-y-4">
          {sectionContent.content.split('\n\n').map((block, i) => {
            if (block.startsWith('#### ')) {
              return (
                <h4 key={i} className="text-xs font-bold uppercase tracking-wider text-neutral-800 mt-4 mb-1 font-mono">
                  {block.replace('#### ', '')}
                </h4>
              );
            }
            if (block.startsWith('### ')) {
              return (
                <h3 key={i} className="text-sm font-bold text-neutral-900 mt-5 mb-2 font-mono flex items-center gap-2">
            ...
            if (block.startsWith('```')) {
              ...
              return (
                <pre key={i} className="p-4 rounded-lg bg-neutral-900 text-neutral-100 font-mono text-xs overflow-x-auto my-3 leading-relaxed border border-neutral-800">
                  {cleanCode}
                </pre>
              );
            }
            if (block.startsWith('> ')) {
              return (
                <blockquote key={i} className="border-l-4 border-emerald-500 bg-emerald-50/60 p-3 rounded-r-lg text-xs italic text-emerald-950 my-3">
                  {block.replace(/^>\s*/, '')}
                </blockquote>
              );
            }
            if (block.startsWith('|')) {
              ...
              return (
                <div key={i} className="overflow-x-auto my-4 border border-neutral-200 rounded-lg shadow-xs">
                      ...
                      {rows.map((r, rIdx) => {
                        ...
                        return (
                          <tr
                            ...
                            className={
                              rIdx === 0
                                ? 'bg-neutral-100 font-mono font-semibold border-b border-neutral-200 text-neutral-900'
            ...
            if (block.startsWith('- ')) {
              ...
              return (
                <ul key={i} className="list-disc pl-5 space-y-1.5 my-2 text-xs text-neutral-700">
                  {items.map((item, idx) => (
